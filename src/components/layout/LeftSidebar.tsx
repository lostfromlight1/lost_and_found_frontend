...
export default function LeftSidebar({ onPostClick }: LeftSidebarProps) {
  ...
  const unreadCount = isLoggedIn ? unreadData?.unreadCount || 0 : 0;
  ...
  return (
    <>
      <div className="flex flex-col h-full w-full pt-8 pb-6 px-2 xl:px-4 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <Link
          href="/dashboard"
          className="flex items-center justify-center xl:justify-start mb-8 xl:ml-3 cursor-pointer w-fit mx-auto xl:mx-0 opacity-90 hover:opacity-100 transition-opacity"
        >
          <div className="flex items-center gap-3">
            ...
            <span className="hidden xl:block font-serif font-bold text-3xl tracking-tight text-slate-900">
              Back2U
            </span>
          </div>
        </Link>
          ...
          {visibleNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className="flex justify-center xl:justify-start w-full"
              >
                <div
                  className={`flex items-center justify-center xl:justify-start gap-5 p-3 xl:px-4 xl:py-3 rounded-full cursor-pointer transition-all hover:bg-slate-200/50 w-fit xl:w-full ${isActive
        ...
        <div className="mt-auto pt-4 flex flex-col gap-2 w-full border-t border-slate-200/60">
          {!isLoggedIn ? (
            <Link
              href="/login"
              className="flex items-center justify-center xl:justify-start gap-5 p-3 xl:px-4 xl:py-3 rounded-full cursor-pointer w-fit mx-auto xl:mx-0 xl:w-full text-slate-500 hover:text-slate-900 hover:bg-slate-200/50 transition-colors"
            >
              <LogIn size={26} strokeWidth={1.5} />
              <span className="hidden xl:block text-[18px] font-normal">
                Sign In
              </span>
            </Link>
          ) : (
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                sideOffset={12}
                className="w-56 rounded-2xl shadow-sm border-slate-200 p-2"
              >
                <DropdownMenuGroup>
