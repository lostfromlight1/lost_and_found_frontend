...
import { Home, Bell, Bookmark, Search, Edit3, ShieldCheck, LogIn } from "lucide-react";
...
// 1. Define a custom interface to tell TypeScript about your custom user fields
...
export default function LeftSidebar({ onPostClick }: LeftSidebarProps) {
  ...
  // 2. Safely cast the session user to our ExtendedUser instead of 'any'
  const user = session?.user as ExtendedUser | undefined;
  
  // Now TypeScript knows that role, roles, and email might exist
  ...
  const navItems = [
    ...
    { name: "Notifications", href: "/notifications", icon: Bell, isPublic: false },
  ...
  return (
    <div className="flex flex-col h-full w-full pt-8 pb-6 px-2 xl:px-4 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <div className="mt-auto pt-4 flex flex-col gap-4 w-full border-t border-slate-100/50">
        {!session ? (
          ...
          <Link href="/settings" className="group">
            <div className="flex items-center justify-center xl:justify-start gap-4 px-3 py-2 cursor-pointer w-fit transition-all">
              <Avatar className="w-10 h-10 border border-slate-200 group-hover:ring-2 ring-slate-300 transition-all">
                {/* 3. Use `user?.` instead of `session.user.` to satisfy TypeScript's strict null checks */}
                <AvatarImage src={user?.image || user?.avatarUrl || undefined} alt={user?.name || "User"} />
                <AvatarFallback className="bg-slate-100 text-slate-600 font-semibold">
                  {user?.name?.charAt(0).toUpperCase() || "U"}
                </AvatarFallback>
              </Avatar>
              <div className="hidden xl:flex flex-col flex-1 truncate text-slate-500 group-hover:text-slate-900 transition-colors">
                <span className="font-semibold text-[15px] leading-tight truncate">{user?.name || user?.displayName}</span>
                <span className="text-[13px] leading-tight truncate">Settings</span>
              </div>
            </div>
          </Link>
        )}
