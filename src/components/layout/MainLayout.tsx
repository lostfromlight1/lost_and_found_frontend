...
export default function MainLayout({ children, rightSidebar }: MainLayoutProps) {
  ...
  return (
    <div className="flex w-full h-full bg-white rounded-none shadow-sm border-x sm:border border-slate-200 overflow-hidden relative">
      ...
      <main className={`flex-1 h-full w-full overflow-y-auto bg-white relative [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] ${rightSidebar ? 'lg:border-r border-slate-200' : ''}`}>
        {children}
      </main>
      ...
      {rightSidebar && (
        <>
          {/* Floating Action Button (Visible only below lg breakpoint) - NOW BLACK AND SMALLER */}
          <button
